...
class ErrorUtils():
    ...
    def check_mandatory_params_is_empty(parameters, data_dict):
        ...
        for v in parameters:
            print v
            print len(str(data_dict.get(v, "")).strip())
    ...
    def validate_name(name):
        ...
        if not re.match("^[a-z0-9-]*$", name): 
            error_response =  {"error": True, "message": error_messages['InvalidInstanceName']} 
        
        return error_response
               
    @staticmethod
    def resource_exists(inner_message):        
        write_log(inner_message)

        error_message = "{0}, {1}".format(error_messages['ResourceAlreadyExists'], inner_message)
        return error_message
