# Commvault Backup Python file Invocation
...
jobId=$(python /opt/remoteFiles/appPackage/commvault-agent/commvaultbackup.py)
...
then
   ccmLog "$backupType [BACKUP] task is Failed. Please refer service.log for detail info."
    ...
    then
        ...
        ccmLog "$backupType [BACKUP] task is Failed. ${jobId}, Please refer service.log for detail info "
        log "[BACKUP] Failed "
    fi 
fi
