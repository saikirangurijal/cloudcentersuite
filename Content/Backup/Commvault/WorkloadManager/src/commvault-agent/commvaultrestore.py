# Commvault File System/MySQL DB Restore Operations
...
def commcellParams(params):
    ...
    try:
        ...
        if commcellname == commserveName:
           ...
           print("Got Commcell ID successfully")
           return commCellId
    except Exception as e:
...
#Create DB Restore Task

def dbRestoreParams(params,subData):

    restore_url = "http://<<server>>/webconsole/api/CreateTask"
    ...
    restore_request_payload =  '<TMMsg_CreateTaskReq> \
                                       <taskInfo> \
    ...
    dic = {'restore_url':restore_url,'headers':headers,'restore_request_payload':restore_request_payload}
    return dic
...
def fileRestoreParams(params,subData):
    ...
    restore_request_payload = '<DM2ContentIndexing_RetrieveToClientReq mode="2" serviceType="1">' \
...
def checkRestore(dic):
    ...
    r1 = requests.post(dic['restore_url'],data=dic['restore_request_payload'], headers=dic['headers'])
    resp = r1.content

    respRoot = ET.fromstring(resp)
    try:
        if restoreType=='File System':
            jobId = respRoot.attrib["jobId"]
            if jobId:
                print (appName+" Restore is Successful")
        ...
        print (appName+" Restore Failed. ErrorCode:" + errorCode)
        sys.exit(0)
...
def dbAndFileRestore():
    ...
    try:
        ...
        print(appName," Restore Processing Failed", e)


if __name__ == '__main__':
    print(dbAndFileRestore())
