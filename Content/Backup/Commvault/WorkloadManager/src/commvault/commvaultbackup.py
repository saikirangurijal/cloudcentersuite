...
def checkBackup(dic):
    ...
    try:
        ...
        if jobId:
            print(appName + " BackUp is Successfully")
...
def updateInstanceBackupParams(params, subData):
    ...
    instance_request_payload = ''
    with open('/opt/remoteFiles/appPackage/commvault/updateinstance.xml', 'r') as file:
...
def updateDbAndFileParams(params, subData):
    ...
    if appId == "29":
        ...
        excludePath = path.split(',')
        for pathpart in excludePath:
            conpath = conpath + '<excludePath>'+pathpart+'</excludePath>'
    else:
        dbpath = dbName.split(',')
        for pathpart in dbpath:
...
def db_Backup():
    try:
        ...
        print("Db Backup Processing Failed", e)
